...
class CnnPolicy(object):
    def __init__(self, sess, ob_space, ac_space, nbatch, nsteps, reuse=True):
        ...
        nact = ac_space.n
        X = tf.placeholder(tf.float32, [None, nw, nh, nc]) 
...
class ActorCritic(object):
    ...
    # Training code for one iteration.
...
def get_actor_critic(sess, nenvs, nsteps, ob_space, ac_space, policy, should_summary=True):
